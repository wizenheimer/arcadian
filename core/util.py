...
def add_to_admin(app_name=None):
    """
    Utility function to automatically add all models to the admin
    """
    if app_name is None:
        raise ValidationError("App name must be specified")
...
def encrypt(txt):
    """
    Utility function to encrypt a text
    """
    try:
